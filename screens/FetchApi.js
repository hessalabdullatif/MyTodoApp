...
const FetchApi = () => {
  ...
  const [refreshing, setRefreshing] = useState(false); // متغير جديد لحالة التحديث

  // فصلنا الفيتش لدالة لحالها عشان ننادها من مكانين
  const fetchWeather = () => {
    fetch('https://api.open-meteo.com/v1/forecast?latitude=24.7136&longitude=46.6753&current=temperature_2m')
      .then((response) => response.json())
      .then((data) => {
        setTemperature(data.current.temperature_2m);
        setRefreshing(false); // نطفي دائرة التحميل بعد ما توصل البيانات
      });
  };

  // يشتغل مره وحده اول ما تفتح الصفحه
  useEffect(() => {
    fetchWeather();
  }, []);

  // يشتغل كل ما المستخدم يسحب من فوق للتحديث
  const onRefresh = () => {
    setRefreshing(true);
    fetchWeather();
  };
